  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(data.message || "Failed to send message. Please check your configuration and try again.");
      }
    } catch (error) {
              ...
              <>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* Phone */}
                    <div>
                      <label className="block text-sm font-bold text-[#0f1b2e] mb-2 ml-1">Phone Number *</label>
                      <input 
                        type="tel" 
                        name="phone"
                        required
                      ...
                      />
                      {phoneError && (
                        <p className="text-red-500 text-xs font-semibold mt-1 ml-1">{phoneError}</p>
                      )}
                    </div>
                    {/* Subject */}
                    <div>
